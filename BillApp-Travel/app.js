    ...
    lines.forEach(line => {
        ...
        if (match) {
            ...
            if (priceVal > 0 && priceVal < 500) {
                let name = cleanLine.replace(match[0], '').trim(); name = name.replace(/^(\d+\s*x?\s*)/i, '').trim(); name = name.replace(/^[^\w\s]+/g, '').trim(); 
                if (name.length > 2) { items.push({ id: 'item_' + idCounter++, name: name, price: priceVal, assignedTo: [] }); }
            }
        }
    }); return items;
}
...
document.getElementById('btn-done').addEventListener('click', () => { manualSubtotalInput.value = ''; manualTaxInput.value = ''; autoResizeInput(manualSubtotalInput); autoResizeInput(manualTaxInput); tipDialControl.setValue(5); splitDialControl.setValue(1); if (!currentTripMode) navigateTo('page-home'); });
...
function initPaintbrushMode() {
    ...
    if (globalParsedItems && globalParsedItems.length > 0) {
        ...
        parsedItemsData = [{ id: 'item_manual', name: "Manual Item (OCR Failed)", price: scannedSubtotal, assignedTo: [] }];
    }
...
function renderScannedItems() {
    ...
    parsedItemsData.forEach((item) => {
        ...
        itemRow.addEventListener('click', () => {
            ...
            const userIndex = item.assignedTo.indexOf(currentBrushUser);
            if (userIndex === -1) { item.assignedTo.push(currentBrushUser); } else { item.assignedTo.splice(userIndex, 1); }
            renderScannedItems(); updateItemizedMath();
        });
